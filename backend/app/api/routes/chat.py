...
async def chat_endpoint(request: ChatRequest):
    if not session_manager.has_documents():
        raise HTTPException(status_code=400, detail="No documents uploaded. Please upload a PDF before asking questions.")

    retrieved_docs = vector_store_service.search(request.question, top_k=4)
    answer, citations = llm_service.generate_rag_response(
